...
public class SAMLTokenProcessor implements Processor {
    ...
    public Assertion handleSAMLToken(Element token) throws WSSecurityException {
        boolean result = false;
        Assertion assertion = null;
        try {
            assertion = (Assertion) XMLObjectProviderRegistrySupport.getUnmarshallerFactory().getUnmarshaller(token).unmarshall(token);
            result = true;
            if (log.isDebugEnabled()) {
